...
2. 以下の条件で重複をチェック（いずれかに当てはまれば重複）：
   - 同一 `id` のイベントが既に存在する
   - 同一 `link` かつ同一 `start`・`end`（開催期間が完全に同じ）のレコードが既に存在する

3. **週分割の例外**: 同一公式記事を週ごとに複数レコードにする場合は `id` を週ごとに分ける（例: `_week1` / `_week2`）。同一 `link` でも別 `id` なら上記の重複に該当しない限り登録する。
...
#### `end`フィールド
- **カレンダー上の含む最終日**を YYYY-MM-DD で設定（アプリのモーダル・一覧の「〜」表示と一致させる）
- **3:59 ルール**: 終了が「Y/Y(曜日) 3:59」のとき、`end` は **Y の前日**。例: 11/4(月) 3:59 まで → `end: "2025-11-03"`
- **週次境界**: 「月曜 4:00 ～ 翌週月曜 3:59」の1ブロックは、`start` をその週の月曜、`end` を **翌週月曜 3:59 の前日＝日曜** にする。例: 4/6(月) 4:00 ～ 4/13(月) 3:59 → `start: "2026-04-06"`, `end: "2026-04-12"`
- **複数週**（1週目・2週目…）はレコードを分割し、週ごとに別 `id`・タイトルに `（1週目）` などを付ける（詳細は下記「日付変換の詳細ルール」）
...
- `data/events_nmd.json`: ニュームーンデー（先行分は東京の新月日を基準にした暦年登録。公式ニュース公開後に差分があれば修正）
- `data/events_gsd.json`: グッドスリープデー（先行分は東京の満月日を基準にした暦年登録。公式ニュース公開後に差分があれば修正）
...
### 必須チェック項目
1. **重複チェックが必須**: すべてのイベントファイルを確認し、上記「3. 重複チェック」の条件に従う（週分割による同一 `link` の複数件は許容）
...
- **`start` / `end` の意味**: 他イベントと同様、JSON の `end` は **カレンダー上で含む最終日**。公式の「Y/Y(曜) 3:59」までの区間なら **`end` は Y の前日**（3:59 ルール）。NMD/GSDは新月日・満月日を挟んだ **ちょうど3日間**（`end - start` が2日＝3暦日）になるよう登録する
- **先行登録（公式未発表年）**: 東京ローカルの新月／満月日（例: timeanddate 東京の月相）を基準に、`start = 当日の前日`、`end = 当日の翌日` で3日ブロックを作る。公式記事の開催期間とずれたら、その月のレコードだけ差し替える
...
### FullCalendar（本アプリ）との関係
- JSON の `end` は **含む最終日**（上記 `end` フィールドの定義どおり）
- FullCalendar の終日イベントは内部的に **`end` が排他的**（その日の 0:00 で区間終了）のため、`js/main.js` の `loadEvents` で FC に渡す直前に、終日かつ `YYYY-MM-DD` 形式の `end` について **+1 日**して排他的境界に変換する
- モーダル・イベント一覧の日付表示は `originalEnd`（JSON の値）のままなので、利用者には「含む最終日」がそのまま見える

### パターン1: 「X/X(曜日) 4:00～Y/Y(曜日) 3:59」（単一区間）
- **開始日**: X/X を YYYY-MM-DD に変換（年は記事の公開年などから推測）
- **終了日（JSON の `end`）**: Y の **前日**（3:59 終了のためカレンダー上の最終日を含む）

例：
- 「12/1(月) 4:00 ～ 12/8(月) 3:59」→ `start: "2025-12-01"`, `end: "2025-12-07"`
- 「10/27(月) 4:00 ～ 11/3(月) 3:59」→ `start: "2025-10-27"`, `end: "2025-11-02"`

### パターン2: 週次「1週目・2週目…」（各が月曜 4:00 ～ 翌週月曜 3:59）
- **各週を別レコード**にする。1本の長い `start`～`end` にまとめない
- 各レコードの `start` / `end`: その週の **月曜 ～ 翌週月曜 3:59 の前日（日曜）**

例（2週イベント）：
- 1週目: 4/6(月) 4:00 ～ 4/13(月) 3:59 → `start: "2026-04-06"`, `end: "2026-04-12"`
- 2週目: 4/13(月) 4:00 ～ 4/20(月) 3:59 → `start: "2026-04-13"`, `end: "2026-04-19"`
- `id` は週ごとに分ける（例: `_week1` / `_week2`）。`title` に `（1週目）` `（2週目）` を付けて区別する
- `allDay: true` として扱う（時刻は本文に残し、JSON は日付のみでよい）
